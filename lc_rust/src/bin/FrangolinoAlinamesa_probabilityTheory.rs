...
fn main() {
    ...
    const MOD:i32 = 1_000_000_007i32;

    const REV_2:i32 = (MOD + 1) / 2;
    let mut cur = 0i64;
    let mut ans:Vec<i64> = vec![0; n + 1];
    x.reverse();
    for (_, &v) in x.iter().enumerate() {
        ans[v as usize] = (ans[v as usize] + cur * REV_2 as i64) % MOD as i64;
        cur = (cur + v) * REV_2 as i64 % MOD as i64;
    }
    ans[1] = (ans[1] + cur) % MOD as i64;
}